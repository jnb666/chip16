...
;0x06 - Submenu or subroutine address (optional)
...
;0x0004 - Run subroutine on activation
;
;Scenes
;0 - Booting scene
;1 - Main scene
...
scene_standard:									;Standard CMOS features scene
	...
	ldi rb, 20									;Load string Y coordinate
	...
	call check_for_back							;Check if B is pressed for return

	vblnk

	jmp scene_selector

scene_advanced:									;Advanced CMOS features scene
	cls											;Clear screen
	bgc #1										;Blue background
	...
	ldi ra, 60									;Load string X coordinate
	ldi rb, 20									;Load string Y coordinate
	ldi rc, string_info_advanced				;Load string memory location
	ldi rd, 0									;Don't show selector
	call draw_string							;Draw string on screen
...
d_m_start:
	...
	ldm r8, rc									;Load menu type to scratchpad
	...
	mov r9, rc 									;Copy string pointer to scratchpad
	addi r9, 8									;Get scene address
...
d_m_input_handle:								;Handle input for menu functions
	tsti r0, PAD_A								;Check if A button is pressed
	jz d_m_draw									;Continue drawing


d_m_activate:									;Activate assigned menu function
	subi r8, 1									;Parent menu
	;jz d_m_jump_scene

	subi r8, 1									;Value selector
	;jz d_m_jump_scene

	subi r8, 1									;Change scene
	jz d_m_jump_scene

	jmp d_m_draw

d_m_jump_scene:									;Jump to scene 
	ldm r6, r9									;Copy next scene
...
check_for_back:									;Check if B is pressed and return to main menu | void CheckForBack();
	tsti r0, PAD_B								;Check if B button is pressed
	jz fret
	ldi r6, 1									;Load main menu scene
	jmp fret
...
menu_main:
	...
	dw 0x0000									;No submenus
	dw 0x0000									;No allowed values
	dw 0x0003									;Scene number

	dw 0x0001									;Change scene type
	dw string_menu_3							;Menu text
	dw 0x0000									;Menu holds no values
	dw 0x0000									;No submenus
	dw 0x0000									;No allowed values
	dw 0x0000									;Scene number
...
string_info_advanced:
	db "Advanced CMOS features"
	db 0x00
